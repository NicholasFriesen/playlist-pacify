...
const client_id = config.keys.client_id; // Your client id
  ...
  if (state === null || state !== storedState) {
    ...
    request.post(authOptions, function(error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        current_client_key = access_token;
        console.log(current_client_key);
...
app.get('/show_playlists', function(req, res) {
  ...
  //  using access token to request playlist information
  var options = {
    url: 'https://api.spotify.com/v1/me/playlists',
      headers: { 'Authorization': 'Bearer ' + current_client_key },
      json: true
    }
    request.get(options, function(error, response, body) {
      if(body) {
        // Do something to all the items
        body.items.forEach(function(element) {
              console.log(element.name);
        }, this);

        // Check the amount grabbed vs total playlists. If there are more to grab, do it fool.
        let limit = parseInt(body.limit);
        let offset = parseInt(body.offset);
        let total = parseInt(body.total);
        ((offset + limit) < total) ? console.log('need to grab more') : console.log ('all done')
      }
    });
});
